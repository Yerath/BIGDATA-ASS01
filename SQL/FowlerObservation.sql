...
CREATE TABLE IF NOT EXISTS `fowler`.`Quantity` (
  ...
  `Measurement_idMeasurement` INT NOT NULL,
  PRIMARY KEY (`idQuantity`),
  INDEX `fk_Quantity_Measurement1_idx` (`Measurement_idMeasurement` ASC),
  CONSTRAINT `fk_Quantity_Measurement1`
    FOREIGN KEY (`Measurement_idMeasurement`)
    REFERENCES `fowler`.`Measurement` (`idMeasurement`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
